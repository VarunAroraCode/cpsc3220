...
// CPSC/ECE 3220 Fall 2020 resource allocation code
//
...
resource_t * resource_init( int type, int total ){
    ...
    rc = pthread_mutex_init(&r->mutexLock, NULL);
    if(rc != 0){
        resource_error(3);
    }
...
void resource_reclaim( resource_t *r ){
    ...
    pthread_mutex_destroy(&r->mutexLock);
...
int resource_allocate( struct resource_type_tag *self, int tid ){
    ...
    while(self -> available_count == 0){
        pthread_cond_wait(&self->cond, &self->mutexLock);
    }
